...
class Fruit internal constructor(mapObject: MapObject, world: TiledWorld) : GameObject(mapObject, world) {
    ...
    init {
        ...
        pluckedSprt = mapObject.properties.get("pluckedPath", String::class.java)?.let {
            val pluckedTex = Texture(it)
            val pluckedRow = mapObject.properties.get("pluckedRow", Int::class.java) ?: 0
            val pluckedCol = mapObject.properties.get("pluckedColumn", Int::class.java) ?: 0
            val pluckedTileSize = mapObject.properties.get("pluckedTileSize", Int::class.java) ?: pluckedTex.height
            val split = TextureRegion.split(pluckedTex, pluckedTileSize, pluckedTileSize)
            Sprite(split[pluckedRow][pluckedCol])
            //}?:sprt.also { Gdx.app.debug(TAG, "No plucked texture detected; using normal texture.") }
            //}?:run { Gdx.app.debug(TAG, "No plucked texture detected; using normal texture."); sprt}
        } ?: sprt.apply { Gdx.app.debug(TAG, "No plucked texture detected; using normal texture.") }
