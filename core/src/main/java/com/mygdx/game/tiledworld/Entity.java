...
public abstract class Entity extends GameObject {
    ...
    private ArrayList<Animation<TextureRegion>> moveAnimations;
    private ArrayList<Animation<TextureRegion>> pullAnimations;
    ...
    Entity(MapObject mapObject, TiledWorld world) {
        ...
        moveAnimations = new ArrayList<>(4);
        pullAnimations = new ArrayList<>(4);
        for (int i = 0; i < 4; i++) {
            idleFrames[i] = tmp[rowsMove[i]][1];
            moveAnimations.add(i, new Animation<>(0.2f, tmp[rowsMove[i]]));
            moveAnimations.get(i).setPlayMode(PlayMode.LOOP_PINGPONG);
            if (hasPullAnimation) {
                pullAnimations.add(i, new Animation<>(0.2f, tmp[rowsPull[i]]));
